...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("C{}".format(i), row)])
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("C{}".format(i), scope)])
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    mFlights = planes_problem.maintenance_flights
    mFreq = planes_problem.min_maintenance_frequency
    planeVars = []
    clist = []
    nC = 1
    for p in planes:
        f = len(planes_problem.can_fly(p))
        for i in range(f):
            name = "{}.{}".format(p, nC)
            if i == 0:
                dom = list(planes_problem.can_start(p))
            else:
                dom = list(planes_problem.can_fly(p))
            dom.append("none.{}".format(nC))
            planeVar = Variable(name, dom)
            planeVars.append(planeVar)
            nC += 1
    a = 1
    clist.extend([AllDiffConstraint("C{}".format(a), planeVars)])
    a += 1
    clist.extend([NValuesConstraint("C{}".format(a), planeVars,
                                              flights, len(flights), 999999)])
    a += 1
    for plane in planes:
        m = []
        for v in planeVars:
            if plane in v.name():
                m.append(v)
        subSequences = map(list, zip(*(m[i:] for i in range(mFreq))))
        if len(m) < mFreq:
            continue
    ...
    for plane in planes:
        ...
        for i in range(len(m) - 1):
            ...
            for flight in firstdomain:
                # for x in planes_problem.can_follow:
                #     if x[0] == flight:
                #         narrowlist

                narrowlist += [x for x in planes_problem.can_follow if
                               x[0] == flight]
            for n in narrowlist:
                dest = n[1]
            ...
            if i == 0:
                constraintTuples += [[firstnone, secondnone]]
            scope = [first, second]
            clist.extend(
        ...
        if len(m) == 1:
            ...
            clist.extend(
                [TableConstraint("C{}".format(a), scope, constraintTuples)])
        a += 1
    csp = CSP("Planes", planeVars, clist)
